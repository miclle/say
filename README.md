...
`say` 是一个用 Go 编写的 macOS 文档与网页朗读终端程序。它读取本地 UTF-8 文本文档，或下载 HTTP(S) 网页并用 [readability.go](https://github.com/miclle/readability.go) 提取正文，再按自然大段落和 TTS 单次调用上限组织内容。第一句音频就绪后立即显示并播放，后续句子和段落按顺序在后台合成。播放过程中可以暂停，也可以按整句或整章前后跳转。交互式启动时可以在 TUI 中选择 macOS 系统 TTS 或实验性的 Microsoft Edge TTS。
...
- 第一句音频就绪后由 AVFoundation 立即播放；同一章节的后续句子和后续章节继续按文档顺序后台合成，无需等待整个章节或整篇文档。
...
- 连续同方向的积压按键会合并定位，避免长按时逐个加载中间音频；近期句子播放器使用有容量上限的缓存。未变化的界面不重绘，章节变化时整帧写出；选中状态先显示，再切换原生音频。切换时提前准备目标播放器，停止旧音频；用户按空格仍保留原位置暂停／恢复。
- 按 `Ctrl-C` 可中断当前播放。
...
上下键切换的是列表中的整个逻辑章节，不是句子；第一章按上键、最后一章按下键不改变播放位置。目标章节尚未合成时会显示 buffering，第一句音频就绪后从章首播放或停留在暂停状态；等待期间可以继续按方向键调整目标，左右键从待播放的目标句继续移动。全部章节和句子边界在音频合成前已确定，混用方向键的结果不受合成进度影响。音频仍按文档顺序后台合成，因此远处尚未准备的章节仍需等待，缓存不会消除首次合成的耗时。
