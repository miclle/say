...
`say` 是一个用 Go 编写的 macOS 文档朗读终端程序。它读取本地 UTF-8 文本文档，按自然句和 TTS 单次调用上限切分内容，在终端显示当前句子后调用系统语音朗读。
...
- 优先按中英文完整句子切分；单句过长时，再从逗号、分号、冒号、空格等位置切分。
- 默认保证每次 TTS 调用不超过 500 个 Unicode 字符，可按服务限制调整。
- 每个语音片段都会先输出到终端，再开始播放；播放完成后显示状态。
- 按 `Ctrl-C` 可中断当前播放。
